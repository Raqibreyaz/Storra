...
export default function UsersSection() {
  ...
  const actionBtnBase =
    "py-1.5 px-3 text-[13px] border-none rounded-lg font-medium text-white cursor-pointer transition-all duration-200 hover:shadow-md active:scale-95";
            ...
            {isLoading ? (
              <tr>
                <td
                  ...
                  className="text-center p-10 text-gray-500 dark:text-gray-400 italic"
                >
                  Loading users...
                </td>
              </tr>
              ...
              users.map((user) => {
                ...
                return (
                  <tr
                    key={user._id}
                    className={`${isDeleted ? "bg-red-50/50 dark:bg-red-900/10" : "hover:bg-gray-50/50 dark:hover:bg-gray-700/20"} transition-colors`}
                  >
                      ...
                      <div className="flex flex-col gap-1.5 mt-1">
                        <div className="flex justify-between items-center">
                          <span className="text-xs font-medium">
                            {formatSize(user.usedStorageInBytes)} /{" "}
                            {formatSize(user.maxStorageInBytes)}
                          </span>
                          <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500">
                            {(
                              ((user.usedStorageInBytes || 0) /
                                (user.maxStorageInBytes || 1)) *
                              100
                            ).toFixed(1)}
                            %
                          </span>
                        ...
                        <div className="w-full h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(((user.usedStorageInBytes || 0) / (user.maxStorageInBytes || 1)) * 100, 100)}%`,
                            }}
                          />
                        </div>
                      </div>
                        ...
                        <div className="flex gap-2 flex-wrap items-center">
                          {isDeleted ? (
                            isOwner && (
                              <>
                                <button
                                  className={`${actionBtnBase} bg-emerald-500 hover:bg-emerald-600`}
                            ...
                            <>
                              ...
                              <button
                                className={`${actionBtnBase} bg-orange-500 hover:bg-orange-600`}
